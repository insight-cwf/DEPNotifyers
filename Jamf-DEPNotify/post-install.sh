...
INSTALLER_BASE_STRING="dep-notify-enrollment"
INSTALLER_SCRIPT_NAME="${INSTALLER_BASE_STRING}-installer.sh"
INSTALLER_SCRIPT_PATH="/tmp/${INSTALLER_SCRIPT_NAME}"
UNINSTALLER_SCRIPT_NAME="${INSTALLER_BASE_STRING}-uninstaller.sh"
UNINSTALLER_SCRIPT_PATH="/tmp/${UNINSTALLER_SCRIPT_NAME}"
...
# Install the package
/usr/sbin/installer -pkg ${HERE}/${DEPNOTIFY_PKG_NAME} -target $3
...
# The following will create the script file to uninstall the LaunchDaemon and
# installer script.
...
# --Execute Command: Whatever your $UNINSTALLER_SCRIPT_PATH is set to.
#
# In your DEPNotify.sh script, include the policy near the end of your
# POLICY_ARRAY.
#
# Paste your script's contents between the two "ENDOFUNINSTALLERSCRIPT" lines.
# NOTE: Make sure to leave a full return at the end of the Script content
# before the last "ENDOFUNINSTALLERSCRIPT" line.
/bin/echo "Creating ${UNINSTALLER_SCRIPT_PATH}."
(
/bin/cat <<ENDOFUNINSTALLERSCRIPT
#!/bin/zsh
# This is meant to be called by a Jamf Pro policy via trigger
# Near the end of your POLICY_ARRAY in your DEPNotify.sh script

/bin/rm ${HERE}/${DEPNOTIFY_PKG_NAME}
/bin/rm ${INSTALLER_SCRIPT_PATH}

# Note that if you unload the LaunchDaemon this will immediately kill the
# depNotify.sh script
# Just remove the underlying plist file, and the LaunchDaemon will not run
# after next reboot/login.

/bin/rm ${LAUNCH_DAEMON_PATH}
/bin/rm ${UNINSTALLER_SCRIPT_PATH}
/bin/rmdir ${HERE}
...
) > "${UNINSTALLER_SCRIPT_PATH}"

/bin/echo "Setting permissions for ${UNINSTALLER_SCRIPT_PATH}."
/bin/chmod 644 "${UNINSTALLER_SCRIPT_PATH}"
/bin/chown root:wheel "${UNINSTALLER_SCRIPT_PATH}"
